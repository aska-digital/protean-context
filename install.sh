#!/usr/bin/env bash
# protean-context - standalone installer.
# Usage: bash install.sh --target <dir> [--dry-run]
# Bash and coreutils only. No network access. Prints every path it writes.
#
# The four install targets below are the descriptor's installTargets and
# payload; gate 1 compares all three lists byte for byte.
set -euo pipefail
...
PAYLOAD=(
  ...
  "scripts/protean-context"
  "templates/protean-context"
  "gates/protean-context")
